# CopyLens — rectangular OCR clipboard utility.
#
# Hotkey (default Hyper-\) summons a transparent overlay; user drags a
# rectangle; on release, the region is captured via ScreenCaptureKit
# and run through Vision. If text is found it's placed on the
# pasteboard; if not, the cropped image is placed on the pasteboard
# instead. Same gesture, two outputs.

# ─── Project identity ────────────────────────────────────────────────────────
BUNDLE_NAME      := CopyLens
BUNDLE_TYPE      := app
PRODUCT_NAME     := CopyLens.app
BUNDLE_ID        := cc.jorviksoftware.CopyLens
BUILD_SYSTEM     := swiftc
