...
- **Hotkey** — click the field and press the combination you want. Default is `control` `option` `shift` `command` `\` (Hyper-\\); change it to anything that includes `command`, `control` or `option`, or to a function key on its own.
